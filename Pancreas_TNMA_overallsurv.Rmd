...
```{r}
rawdata<-read.csv("overall_surv_age.csv")
```


```{r}

names(rawdata)<-c("id","time","site_specific","delta","7T","7N","7M","7M16","7T16","7N16","8T","8N","8M","year","H_C","age")
...
choose_A<-function(data_TNM){
  newdata<-data_TNM[data_TNM$A%in%c("A1","A2") ,]
  return(newdata)}
data_TNM<-choose_A(data_TNM)
...
```{r}
data_TNM$A<-factor(data_TNM$A,levels=c("A1",
                               "A2"
                               ))
...
########################################################
#################    Don't Change   ######################
vars<-c("T","N","M","A")   # Construct a vector "vars" that contains the name of the factors we are interested in. The names must be exact the same as the factor names in the inputted data.
##############################################################
#############################################################
```

```{r}
elimSize<-12  # set a number "elimSize" so that any combination with size < "elimSize" will be eliminated.
...
write.csv(testdata, file = "Dataset_2_920.csv", row.names = FALSE)
...
png("km_curves_combinations.png", width = 2400, height = 1800, res = 300)
...
if(numComb<2) #add legend for (groups of combinations) <12 for the K-M Curve
{
...
png("cindex_curve919.png", width = 2400, height = 1800, res = 300)  
...
png("dendrogram_919.png", width = 2400, height = 1800, res = 300) 
...
plot(myClust,labels=as.character(label),cex.axis=1.5, xlab=NA,main="",ylab="Dissimilarity",sub='',cex.lab=1.5, cex=min(40/numComb,1.5))
assign<-rect.hclust(myClust, k=finalGroups, border="red") # draw boxes
...
Cindexdata$group_ind <- stage_map[Cindexdata$group_ind]
    ...
    write.table(Cindexdata, file = "EACCD_TNMA_data.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
```{r}
 library(compareC)
compareC(groupdata$time, groupdata$delta, -groupdata$r_group_ind, -groupdata$r_group_ind)
...
stage1<-mydata_old_stage%>%
  filter(r_group_ind=="1")
nstage1<-as.numeric(nrow(stage1))
stage2<-mydata_old_stage%>%
  filter(r_group_ind=="2")
nstage2<-as.numeric(nrow(stage2))
stage3<-mydata_old_stage%>%
  filter(r_group_ind=="3")
nstage3<-as.numeric(nrow(stage3))
stage4<-mydata_old_stage%>%
  filter(r_group_ind=="4")
nstage4<-as.numeric(nrow(stage4))
stage5<-mydata_old_stage%>%
  filter(r_group_ind=="5")
nstage5<-as.numeric(nrow(stage5))
stage_sizes<-c(nstage1,nstage2,nstage3,nstage4,nstage5)
...
png("survival_plot919.png", width = 2400, height = 1800, res = 300)
#pdf(file="3_3.pdf",width=9, height=6)
plot(grouptest,ylim=c(0, 1.0),
...
deaths  <- integer(length(targets))

for (i in seq_along(targets)) {
  subset_i <- mydata_old_stage[ mydata_old_stage$comb == targets[i], ]
  
  counts[i] <- nrow(subset_i)
  
  deaths[i] <- sum(subset_i$time < 60, na.rm = TRUE)
}
...
result_sorted <- result[order(-result$Count), ]
...
```{r}
groupdata_1_and_2<-groupdata%>%
  filter(r_group_ind==1|r_group_ind==2)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_1_and_2)
surv_diff
```


```{r}
groupdata_1_and_3<-groupdata%>%
  filter(r_group_ind==1|r_group_ind==3)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_1_and_3)
surv_diff
```

```{r}
groupdata_1_and_4<-groupdata%>%
  filter(r_group_ind==1|r_group_ind==4)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_1_and_4)
surv_diff
```

```{r}
groupdata_1_and_5<-groupdata%>%
  filter(r_group_ind==1|r_group_ind==5)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_1_and_5)
surv_diff
```


```{r}
groupdata_3_and_2<-groupdata%>%
  filter(r_group_ind==3|r_group_ind==2)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_3_and_2)
surv_diff
```

```{r}
groupdata_4_and_2<-groupdata%>%
  filter(r_group_ind==4|r_group_ind==2)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_4_and_2)
surv_diff
```

```{r}
groupdata_2_and_5<-groupdata%>%
  filter(r_group_ind==2|r_group_ind==5)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_2_and_5)
surv_diff
```


```{r}
groupdata_3_and_4<-groupdata%>%
  filter(r_group_ind==3|r_group_ind==4)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_3_and_4)
surv_diff
```

```{r}
groupdata_3_and_5<-groupdata%>%
  filter(r_group_ind==3|r_group_ind==5)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_3_and_5)
surv_diff
```

```{r}
groupdata_4_and_5<-groupdata%>%
  filter(r_group_ind==4|r_group_ind==5)
surv_diff <- survdiff(Surv(time, delta) ~ r_group_ind, data = groupdata_4_and_5)
surv_diff
```
...
```{r}
groupdatat1<-groupdata%>%
  filter(T=="T1")

nrowt1<-nrow(groupdatat1)
nrow_total<-nrow(groupdata)
percentage<-nrowt1/nrow_total
print(percentage)
